class WebhooksController < Telegram::Bot::UpdatesController
  ...
  def register(person_number = nil, *)
    ...
    ::Commands::Register.perform(payload, &respond_with_text_and_save_context(:register))
  end

  def checkin!(*)
    return respond_with_message(:already_checked_in) if current_user.checked_in?

    ::Commands::CheckIn.perform(payload, &respond_with_text_and_save_context(:checkin!))
  end

  def checkout!(*)
    return respond_with_message(:already_checked_out) if current_user.checked_out?

    ::Commands::CheckOut.perform(&respond_with_text_and_save_context(:checkout!))
  end
